<template>
  <div class="purchase-order-container">
    ...
    <div class="purchase-order">
      <el-form :model="queryParams"
        ...
        <el-form-item label="采购负责人"
          prop="purchaseUserId">
          <el-select v-model="queryParams.purchaseUserId"
            filterable
            remote
            ...
            <el-option v-for="item in purchaseUserSearchList"
              :key="item.userId"
              :label="item.nickName"
              :value="item.userId" />
          </el-select>
        ...
        <el-form-item label="销售负责人"
          prop="salesUserId">
          <el-select v-model="queryParams.salesUserId"
            filterable
            clearable
            remote
            ...
            :loading="selectLoading">
            <el-option v-for="item in salesUserSearchList"
              :key="item.userId"
              :label="item.nickName"
              :value="item.userId" />
          </el-select>
        ...
        <el-form-item label="品牌名称"
          ...
          <el-select v-model="queryParams.brandId"
            ...
            :loading="selectLoading">
            <el-option v-for="item in brandSearchList"
              :key="item.brandId"
        ...
        <el-form-item label="订单号"
          prop="purchaseSn">
          <el-input v-model="queryParams.purchaseSn"
            placeholder="请输入订单号"
            clearable />
        </el-form-item>
        <el-form-item v-hasRole="['admin']"
          label="供应商名称"
          prop="supplierId">
          <el-input v-model="queryParams.supplierId"
        ...
        <el-form-item label="订单状态"
          ...
          <el-select v-model="queryParams.orderState"
            ...
            clearable
            style="width: 240px">
            <el-option v-for="dict in order_state"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value" />
          </el-select>
        ...
        <el-form-item label="订单状态更新时间">
          <el-date-picker v-model="daterangeOrderStateUpdateTime"
            style="width: 240px"
      ...
      <el-table v-loading="loading"
        ...
        <el-table-column label="品牌"
          align="center"
          prop="brandName" />
        <el-table-column label="采购负责人"
          align="center"
          prop="purchaseUserName" />
        <el-table-column label="销售负责人"
          align="center"
          prop="salesUserName" />
        <el-table-column label="订单状态"
          ...
          <template #default="scope">
            <dict-tag :options="order_state"
              :value="scope.row.orderState" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="订单状态更新时间"
          align="center"
          prop="orderStateUpdateTime"
          width="180">
          <template v-slot="scope">
            <span>{{ parseTime(scope.row.orderStateUpdateTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
          </template>
        </el-table-column>
        <el-table-column v-hasRole="['admin']"
          label="供应商"
          align="center"
          prop="supplierId" />
        <el-table-column label="操作"
          ...
          <template v-slot="scope">
            <el-button size="small"
              ...
              v-hasPermi="['purchase:order:edit']">修改</el-button>
    ...
    <!-- 添加或修改订单功能对话框 -->
    <OrderEditDialog ref="orderEditDialogRef"
      :title="dialogTitle"
      :open="true"
      @ok="handleQuery"></OrderEditDialog>
  </div>
</template>

<script setup name="Order">
import { listOrder, getOrder, delOrder, addOrder, SearchUser, searchBrand } from "@/api/purchase/order";
import { reactive } from "vue";
import OrderEditDialog from "./orderEditDialog";
import omsMessage from "./omsMessage.vue";
...
function getList() {
  loading.value = true;
  let pramers = proxy.addDateRange(queryParams.value, daterangeOrderStateUpdateTime.value, 'daterangeOrderStateUpdateTime')
  listOrder(proxy.addDateRange(pramers, createTime.value, 'CreateTime')).then(response => {
...
function handleUpdate(row) {
  ...
  proxy.$refs["orderEditDialogRef"].show(row.purchaseId);
}
