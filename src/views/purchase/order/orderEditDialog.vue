  ...
  <el-dialog :title="title"
    ...
    <el-form ref="orderRef"
      ...
      <el-form-item prop="timProductList">
        <el-table :data="form.timProductList"
          border
          ...
          <el-table-column type="index"
            label="序号"
            width="60" />
          <el-table-column prop="timProductList.model"
            label="型号"
            width="200">
            <template #default="scope">
              <el-form-item :prop="'timProductList.' + scope.$index + '.model'"
                :rules="valueRule">
                <el-select v-model="scope.row.model"
                  ...
                  @change="(val) => { modelSelectChange(val, scope.row) }"
                  :loading="selectLoading">
                  <el-option v-for="item in modelSearchList"
                    :key="item.modelId"
          ...
          <el-table-column prop="productDescription"
            label="产品描述">
            <template #default="scope">
              <el-input :rows="3"
                type="textarea"
          ...
          <el-table-column prop="salesAttachmentList"
            align="“center"
            label="附件"
            width="300">
            <template #default="scope">
              <el-upload v-model:file-list="scope.row.salesAttachmentList"
            ...
            <template #default="scope">
              ...
              <el-tooltip content="添加"
                ...
                <el-button link
                  type="primary"
                  icon="Plus"
                  @click="handleAddOrderItem()"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
    ...
    <div slot="footer"
      ...
      <el-button type="success"
        @click="submitForm(6)">保存草稿</el-button>
      <el-button type="primary"
        @click="submitForm(1)">确 定</el-button>
    </div>
  </el-dialog>
</template>
...
function handleDeleteOrderItem(index) {
  proxy.$modal.confirm('是否确认删除序号为"' + (index + 1) + '"产品?').then(function () {
